...
class PsdExporter {
    ...
    createPsdLayerAndMaskInfo(layers, width, height, options) {
        const layerRecords = [];
        const layerImages = [];
    ...
    createLayerRecord(layer, top, left, bottom, right) {
        const parts = [];
    ...
    createLayerImage(layer, width, height) {
        const rawData = [];
    ...
    async exportEcommercePsd(layers, outputPath, width, height, options) {
        try {
            const header = this.createPsdHeader(width, height);
            const colorModeData = this.createPsdColorModeData();
            const imageResources = this.createPsdImageResources();
            ...
            return {
                success: false,
                message: `商业级电商 PSD 导出失败: ${error.message}`
            };
        }
    }
}
