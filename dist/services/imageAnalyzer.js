...
class ImageAnalyzer {
    ...
    checkWhiteBackground(data, info) {
    ...
    assessBackgroundComplexity(data, info) {
    ...
    assessSubjectComplexity(data, info) {
        let edgeCount = 0;
    ...
    getCenterRegionStats(data, info) {
    ...
    detectPerson(data, info) {
        return this.hasSkinTones(data, info);
    }
    detectHairLikeTextures(data, info) {
    ...
    detectTransparentMaterial(data, info) {
    ...
    checkEcommerceImage(info, productType) {
        const standardSizes = [
            { w: 1000, h: 1000 },
            { w: 1200, h: 1200 },
            { w: 1500, h: 1500 },
        ...
        const isStandardSize = standardSizes.some(s => Math.abs(info.width - s.w) < 200 && Math.abs(info.height - s.h) < 200);
