...
import { Layer, SplitResult } from '../types';
...
export class LayerSplitter {
  ...
  private async detectLayers(imageBuffer: Buffer): Promise<Layer[]> {
    const layers: Layer[] = [];
  ...
  private async extractVisualBlocks(imageBuffer: Buffer): Promise<Layer[]> {
    const layers: Layer[] = [];
  ...
  async splitImage(imagePath: string): Promise<SplitResult> {
    try {
