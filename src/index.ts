...
import { CutoutSolutionSelector } from './services/cutoutSolver';
import { EcommerceLayerManager } from './services/ecommerceLayerManager';
import { Layer, ActionContext, MessageContext, FileAttachment } from './types';
import { AIModelConfig, AI_PROVIDERS, SegmentationResult } from './types/aiModels';
import { ImageAnalysis, CutoutSolution, DetailedLayer, ProcessingReport } from './types/photoshop';
...
const cutoutSelector = new CutoutSolutionSelector();
const layerManager = new EcommerceLayerManager();

let originalSize: { width: number; height: number } | null = null;
let currentLayers: Layer[] = [];
let currentDetailedLayers: DetailedLayer[] = [];
let lastSegmentationResult: SegmentationResult | null = null;
let lastAnalysis: ImageAnalysis | null = null;
let lastCutoutSolution: CutoutSolution | null = null;
let lastProcessingReport: ProcessingReport | null = null;
...
export const actions = {
  ...
  'photoshop-cutout': {
    ...
    async execute(context: ActionContext) {
      ...
      try {
        ...
        let exportMessage = '';
        if (outputPath) {
          const exportResult = await psdExporter.exportEcommercePsd(
            detailedLayers,
            outputPath,
            report.originalSize.width,
        ...
        return {
          success: false,
          message: `Photoshop抠图处理失败: ${(error as Error).message}`
        };
      }
    }
  },
  ...
  'export-psd': {
    ...
    async execute(context: ActionContext) {
      ...
      const useEcommerceMode = parameters.useEcommerceMode as boolean | undefined;
      ...
      if (useEcommerceMode && currentDetailedLayers.length > 0 && originalSize) {
        ...
        const result = await psdExporter.exportEcommercePsd(
...
function generatePhotoshopGuide(analysis: ImageAnalysis, solution: CutoutSolution | null) {
  const steps: string[] = [];
...
async function saveAttachmentToTempFile(attachment: FileAttachment): Promise<string> {
...
export async function onMessage(context: MessageContext): Promise<void> {
  ...
  if (attachments && attachments.length > 0) {
    ...
    if (imageAttachment) {
      ...
      try {
        ...
        await psdExporter.exportEcommercePsd(
          detailedLayers,
          psdOutputPath,
          report.originalSize.width,
          report.originalSize.height,
        ...
        const psdAttachment: FileAttachment = {
          id: `psd_${Date.now()}`,
        ...
        logger.error(`处理图片失败: ${(error as Error).message}`);
        await reply(`抱歉，处理图片时出错: ${(error as Error).message}\n请稍后重试或联系技术支持。`, {
